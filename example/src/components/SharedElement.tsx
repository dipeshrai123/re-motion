...
import { useValue } from './react-ui-animate/hooks';
import {
  withSpring,
  withTiming,
  withSequence,
  withEase,
} from './react-ui-animate/controllers';
import { animate } from './react-ui-animate/animate';
import { useDrag } from '@use-gesture/react';
import { MountedBlock } from './react-ui-animate/Modules';
...
export default function SharedElement() {
  ...
  const bind: any = useDrag(({ down, movement: [mx, my] }) => {
    translateY.value = down ? withEase(clamp(my, 0, 300)) : withSpring(0);
    translateX.value = down ? withEase(mx) : withSpring(0);
